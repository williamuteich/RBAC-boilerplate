...
export function HeroMockupStack() {
  ...
  const [showLeftTip, setShowLeftTip] = useState(true);
  const [showRightTip, setShowRightTip] = useState(true);
    ...
    <div className="relative w-full max-w-[420px] h-[380px] sm:h-[440px] flex items-center justify-center mx-auto select-none mt-6 lg:mt-0">
      ...
      <div className="absolute left-[3%] sm:left-[8%] w-[180px] h-[320px] bg-black rounded-[30px] border-[5px] border-neutral-900 shadow-[0_15px_30px_rgba(0,0,0,0.4)] p-1 overflow-hidden z-10 -rotate-6 transition-all hover:rotate-0 hover:scale-[1.02] hover:z-25 duration-500">
        <div className="relative w-full h-full overflow-hidden rounded-[24px]">
          <div
            onScroll={(e) => setShowLeftTip(e.currentTarget.scrollTop < 15)}
            className="w-full h-full bg-[#121212] text-white p-2 flex flex-col justify-between text-left relative select-none pt-7 pb-4 px-2.5 scrollbar-hidden overflow-y-auto touch-pan-y"
          >
          ...
          {showLeftTip && (
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-40 bg-rose-500 text-white p-1 rounded-full shadow-md pointer-events-none animate-bounce flex items-center justify-center">
      ...
      <div className="absolute right-[3%] sm:right-[8%] w-[190px] h-[340px] bg-black rounded-[32px] border-[6px] border-neutral-900 shadow-[0_20px_50px_rgba(0,0,0,0.3)] p-1 overflow-hidden z-20 rotate-6 transition-all hover:rotate-0 hover:scale-[1.02] duration-500">
        <div className="relative w-full h-full overflow-hidden rounded-[24px]">
          <div
            onScroll={(e) => setShowRightTip(e.currentTarget.scrollTop < 15)}
            className="w-full h-full bg-[#FAF9FF] p-2.5 flex flex-col gap-3 text-left relative scrollbar-hidden overflow-y-auto text-[#2D2A4A] pb-4 touch-pan-y"
          >
